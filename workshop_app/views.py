...
def dashboard_api(request):
    ...
    workshops = (
        Workshop.objects.select_related("workshop_type", "coordinator", "instructor")
        .order_by("date")
    )
    ...
    stats = {
        ...
        "upcoming_workshops": workshops.filter(date__gte=today).count(),
        "workshop_types": workshop_types.count(),
    }
    ...
    recent_workshops = [
        {
            ...
            "coordinator": workshop.coordinator.get_full_name() or workshop.coordinator.username,
            "instructor": (
                workshop.instructor.get_full_name() or workshop.instructor.username
            ) if workshop.instructor else "Not assigned yet",
        }
        for workshop in workshops[:8]
    ]
    ...
    type_summary = [
        {
            ...
            "resource_count": AttachmentFile.objects.filter(workshop_type=item).count(),
        }
        for item in workshop_types[:8]
    ]

    if not type_summary:
        data_root = Path("workshop_app") / "data"
        fallback_types = []
        if data_root.exists():
            for folder in sorted([item for item in data_root.iterdir() if item.is_dir()]):
                resources = sorted(file.name for file in folder.iterdir() if file.is_file())
                fallback_types.append({
                    "id": folder.name.lower().replace(" ", "-"),
                    "name": folder.name.replace("_", " "),
                    "duration": len(resources) or 1,
                    "description": f"Includes {len(resources)} workshop resource file(s) bundled with the project.",
                    "resource_count": len(resources),
                })

        type_summary = fallback_types
        stats["workshop_types"] = len(type_summary)

    if not recent_workshops:
        fallback_dates = [today, today + timedelta(days=3), today + timedelta(days=7)]
        recent_workshops = [
            {
                "id": index + 1,
                "title": item["name"],
                "date": fallback_dates[index % len(fallback_dates)].isoformat(),
                "status": "Ready",
                "duration": item["duration"],
                "coordinator": "FOSSEE Coordinator",
                "instructor": "Instructor to be assigned",
            }
            for index, item in enumerate(type_summary[:3])
        ]
        stats["upcoming_workshops"] = len(recent_workshops)

    return JsonResponse({
        "stats": stats,
        "workshops": recent_workshops,
        "workshop_types": type_summary,
    })
